...
// Cache bust - force refresh on page reload (2026-02-10 fix)
...
function handleLabelColorSelect() {
    ...
    // If trying to change color but label is empty, show warning and prevent change
    ...
    if (select.value === 'custom') {
        ...
        // Add input listener to color picker
        if (picker) {
...
function updateLivePreview() {
    ...
    // Only add labelColor if label has actual value (not just placeholder)
...
function checkLabelValidation() {
    ...
    // Hide warning if label is filled
    if (label) {
...
function copyLiveUrl(event) {
    ...
    navigator.clipboard.writeText(url).then(() => {
        ...
        // Spawn ripple effect at click point - single wave for subtle effect
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('DOM carregado - Gerando preview inicial');
        ...
        fileInput.addEventListener('change', function(e) {
            ...
            reader.onload = function(event) {
                try {
                    ...
                    console.log('SVG carregado, tamanho:', svgContent.length);
                    // Simple base64 conversion
                    const base64 = btoa(unescape(encodeURIComponent(svgContent)));
                    customSvgDataUrl = `data:image/svg+xml;base64,${base64}`;
                    console.log('SVG Data URL criado:', customSvgDataUrl.substring(0, 80));
                    updateLivePreview();
                } catch (error) {
    ...
    inputs.forEach(id => {
        ...
        if (input) {
            input.addEventListener('input', () => {
                updateLivePreview();
                // Check validation when label changes
                if (id === 'label') {
                    checkLabelValidation();
                }
            });
